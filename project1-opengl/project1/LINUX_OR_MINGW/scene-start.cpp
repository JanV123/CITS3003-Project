...
static float camRotUpAndOverDeg= 0; //20; // rotates the camera up and over the centre.
...
//	float angle0= sceneObj.angles[0]; 
//	float angle1= sceneObj.angles[1];
//	float angle2 = sceneObj.angles[2];
	
	mat4 model;
	
	model = Translate(sceneObj.loc) * Scale(sceneObj.scale) * RotateX(sceneObj.angles[0]) * RotateY(sceneObj.angles[1]) * RotateZ(sceneObj.angles[2]);
...
void display( void ) {
    ...
    float angle = camRotUpAndOverDeg;
    float angle2 = camRotSidewaysDeg;
...
*/
    ...
    view = view * (RotateX(angle) * RotateY(angle2));
...
// gluPerspective((float) angle, width/height, near, far); determines which 
// items inside the window are rendered or clipped out
// in this case Frustum is used to describe a perspective matrix that produces
...
//*
		...
		//float rscale;
		//rscale = (float)width / height;
		//sceneHeight = (float)hscale * width;
		
		sceneHeight = (float)width;
		//sceneHeight = (float)height;
		sceneWidth = (float)width;
		
		//sceneWidth = (float)height/1.5;
		yscale = (height - width) / 2.0;
	//ie. width = 100; height = 700; windowHeight = 100, windowWidth = 100; middle would be 300-400 
		ratio = 1.0 * sceneWidth / sceneHeight;
		glMatrixMode(GL_PROJECTION);
		glLoadIdentity();
		glViewport(0, yscale, (float)sceneWidth, (float)sceneHeight);
		
		projection = Frustum(-nearDist*(float)(sceneWidth)/sceneHeight,
                         nearDist*(float)(sceneWidth - width)/sceneHeight,
                         -nearDist, nearDist,
                         nearDist, 100.0);
		...
		sceneHeight = width;
		yscale = (height - width) / 2.0;
		ratio = 1.0 * sceneWidth / height;
		glMatrixMode(GL_PROJECTION);
		glLoadIdentity();
		//glutReshapeWindow(sceneWidth,sceneHeight);
		glViewport(0, yscale, (float)sceneWidth, (float)sceneHeight);
		
		projection = Frustum(-nearDist*(float)(width)/ height,
                         nearDist*(float)(width) / height,
                         -nearDist, nearDist,
                         nearDist, 100.0);
	}
		 ...
		 projection = Frustum(-nearDist*(float)ratio,
                         nearDist*(float)ratio,
                         -nearDist, nearDist,
                         nearDist, 100.0);
	}
/*
	...
	//glViewport(width/2, 0, width/2, height/2);
        ...
        projection = Frustum(-nearDist*(float)ratio,
                         ...
                         nearDist, 100.0);
*/
// if width < height then maintain a square 
// if width = 200, height = 300, then height should = 200
